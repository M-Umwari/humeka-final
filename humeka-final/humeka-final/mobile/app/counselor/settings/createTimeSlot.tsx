...
import { defaultTimeSlotSchema, timeSlotSchema } from "@/validationSchema/timsSlotSchema";
import moment from 'moment-timezone'
import { createDefaultTimeSlot, createTimeSlot } from "@/redux/actions/timeSlotActions";
...
const NewTimeSlot = () => {
    ...
    const handleFromDateChange = (event:DateTimePickerEvent, selectedDate: Date | undefined, setFieldValue: (field: string, value: any, shouldValidate?: boolean) => void) => {
        ...
        const newDate = combineDateAndTime(formik.values.date, currentDate)
        setShowFromPicker(Platform.OS === 'ios'); // Hide picker on Android
        setFieldValue('from', newDate, true); // Update Formik state with the selected date
    };
    ...
    const handleToDateChange = (event:DateTimePickerEvent, selectedDate: Date | undefined, setFieldValue: (field: string, value: any, shouldValidate?: boolean) => void) => {
        ...
        setFieldValue('to', newDate, true); // Update Formik state with the selected date
    };

    const handleDateChange = (event:DateTimePickerEvent, selectedDate: Date | undefined, setFieldValue: (field: string, value: any, shouldValidate?: boolean) => void) => {
        const currentDate = selectedDate || new Date();
        setShowDatePicker(Platform.OS === 'ios'); // Hide picker on Android
        setFieldValue('date', currentDate, true); // Update Formik state with the selected date
    }
            ...
            <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled" className="w-[90%] pb-8">
                <View className="w-full mt-4 items-center">
                    <View className="w-full mb-6">
                        <Text className="mb-1 font-ubuntu">Date</Text>
                        {showDatePicker && <DateTimePicker
                            mode="date"
                            display="spinner"
                            value={formik.values.date!}
                            onChange={(event: DateTimePickerEvent, selectedDate: Date | undefined) => handleDateChange(event, selectedDate, formik.setFieldValue)}
                        />}
                        {!showDatePicker && <Pressable onPress={() => setShowDatePicker(true)} className={`flex-row w-full items-center justify-between h-12 px-2 border ${formik.touched.date && formik.errors.date?'border-red-500':'border-custom-borderGrey'} rounded-md bg-[#f2f2f2]`}>
                        ...
                        {formik.touched.date && formik.errors.date && <Text className="text-red-500 text-xs font-ubuntu mt-2">{formik.errors.date as string}</Text>}
                    </View>
                    <View className="w-full mb-6">
